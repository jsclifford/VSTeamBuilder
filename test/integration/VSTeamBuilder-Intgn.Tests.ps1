...
Describe 'VSTeamBuilder Integration Org Test'{
    ...
    Context 'Add/Remove TBOrg' {
        ...
        It 'Creates new project and uses CSV as template - New-TBOrg' {
            $result = New-TBOrg -ProjectName "$projectName-CSV" -ProjectDescription "The Best Project Ever" -ImportFile "$ResourceRootDir\VSTBImportFile.csv" -NewProject -Verbose:$Verbose
            $success = $false
            if($result){
                $success = $true
            }
        ...
        It 'Removes team project structure from csv file - Remove-TBOrg' {
            $result = Remove-TBOrg -ProjectName "$projectName-csv" -ImportFile "$ResourceRootDir\VSTBImportFile.csv" -Verbose:$Verbose
            $success = $false
            if($result){
                $success = $true
            }
...
Describe "Testing Team Creation and Settings" {
    ...
    Context 'Add and Test Team Settings CSV Basic Version' {
        ...
        It 'Creates a new Team - New-TBTeam' {
            $result = New-TBTeam -Name $TeamName -Description $TeamDescription -TeamCode $TeamCode -TeamPath $TeamRootPath -isCoded -ProjectName $projectName -Verbose:$Verbose
            $result | Should Be True
        }

        It 'Creates a new SubTeam - New-TBTeam' {
            $result = New-TBTeam -Name "$TeamName-Sub" -Description $TeamDescription -TeamCode "$TeamCode-Sub" -TeamPath "MTT" -isCoded -ProjectName $projectName -Verbose:$Verbose
            $result | Should Be True
        }
    ...
    Context 'Team Area' {
        ...
        It 'Sets Team Area Default Setting - Set-TBTeamAreaSetting' {
            $result = Set-TBTeamAreaSetting -AreaPath $AreaPath -TeamName $TeamName -ProjectName $projectName -Verbose:$Verbose
    ...
    Context 'Team Iteration' {
        ...
        It 'Sets Team Iteration Default Setting - Set-TBTeamIterationSetting' {
            $result = Set-TBTeamIterationSetting -IterationName $IterationName -TeamName $TeamName -ProjectName $projectName -Verbose:$Verbose
    ...
    Context 'TFS Permissions and Tokens' {
        ...
        It 'Get Token - Get-TBToken' {
            ...
            $objectId = $object.id
            $token = Get-TBToken -ObjectId $objectId -NsName "Git Repositories" -ProjectName $projectName -Verbose:$Verbose
        ...
        It 'Sets TFS Permission on Git Repo - Set-TBPermission' {
            ...
            $result = Set-TBPermission -TokenObject $token -GroupName "$TeamCode-Contributors" -AllowValue 118 -ProjectName $projectName -Verbose:$Verbose
